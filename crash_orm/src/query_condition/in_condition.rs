...
pub trait InQueryColumn<T: ToSql, U: Entity<U>> {
    fn in_vec(&self, other: Vec<&(dyn IntoColumnValue<T>)>) -> QueryCondition<U>;

    fn not_in_vec(&self, other: Vec<&(dyn IntoColumnValue<T>)>) -> QueryCondition<U>;
}

macro_rules! impl_in_entity_column {
    ($column_type:ty) => {
        impl<U: Entity<U>, R: Column<$column_type, U>> InQueryColumn<$column_type, U> for R {
            fn in_vec(&self, other: Vec<&(dyn IntoColumnValue<$column_type>)>) -> QueryCondition<U> {
                QueryCondition::In(
                    self.get_sql(),
            ...
            fn not_in_vec(&self, other: Vec<&(dyn IntoColumnValue<$column_type>)>) -> QueryCondition<U> {
