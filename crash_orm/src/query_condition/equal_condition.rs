...
use crate::{Entity, QueryCondition, Column, BoundColumnValue};

/// Trait implementing equals operators
pub trait EqualQueryColumn<T: ToSql, U: Entity<U>> {
    fn equals(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;

    fn not_equals(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
}

macro_rules! impl_equal_entity_column {
    ($column_type:ty) => {
        impl<T: Entity<T>, U: Column<$column_type, T>> EqualQueryColumn<$column_type, T> for U {
            fn equals(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<T> {
                QueryCondition::Equals(self.get_sql(), other.get_sql())
            }

            fn not_equals(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<T> {
