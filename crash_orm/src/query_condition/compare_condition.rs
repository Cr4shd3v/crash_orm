...
use crate::{Entity, Column, QueryCondition, BoundColumnValue};

/// Trait implementing comparison operators
pub trait CompareQueryColumn<T: ToSql, U: Entity<U>> {
    fn greater_than(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
    fn greater_equal(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
    fn less_than(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
    fn less_equal(&self, other: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
    fn between(&self, from: &(dyn BoundColumnValue<T>), to: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
    fn not_between(&self, from: &(dyn BoundColumnValue<T>), to: &(dyn BoundColumnValue<T>)) -> QueryCondition<U>;
}

macro_rules! impl_compare_entity_column {
    ($column_type:ty) => {
        impl<U: Entity<U>, R: Column<$column_type, U>> CompareQueryColumn<$column_type, U> for R {
            fn greater_than(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
                QueryCondition::GreaterThan(self.get_sql(), other.get_sql())
            }

            fn greater_equal(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
                QueryCondition::GreaterEqual(self.get_sql(), other.get_sql())
            }

            fn less_than(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
                QueryCondition::LessThan(self.get_sql(), other.get_sql())
            }

            fn less_equal(&self, other: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
                QueryCondition::LessEqual(self.get_sql(), other.get_sql())
            }

            fn between(&self, from: &(dyn BoundColumnValue<$column_type>), to: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
                QueryCondition::Between(self.get_sql(), from.get_sql(), to.get_sql())
            }

            fn not_between(&self, from: &(dyn BoundColumnValue<$column_type>), to: &(dyn BoundColumnValue<$column_type>)) -> QueryCondition<U> {
