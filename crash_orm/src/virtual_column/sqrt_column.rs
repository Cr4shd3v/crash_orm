...
pub trait SqrtVirtualColumn<T: ToSql, U: Entity<U, PRIMARY>, PRIMARY: PrimaryKey> {
    fn sqrt(&self) -> VirtualColumn<T, U, PRIMARY>;
}

macro_rules! impl_sqrt_virtual_column {
    ($column_type:ty) => {
        impl<U: Entity<U, PRIMARY>, R: Column<$column_type, U, PRIMARY>, PRIMARY: PrimaryKey> SqrtVirtualColumn<$column_type, U, PRIMARY> for R {
