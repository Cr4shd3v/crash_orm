...
macro_rules! impl_avg_column {
    ($in_type:ty, $out_type:ty) => {
        #[async_trait]
        impl<T: Entity<T> + Send + Sync + 'static> AvgColumn<$in_type, $out_type, T> for EntityColumn<'_, $in_type, T> {
            async fn avg(&self, connection: &DatabaseConnection, distinct: bool) -> crate::Result<$out_type> {
                let row = connection.query_one(
                    &*format!("SELECT AVG({}{}) FROM {}", if distinct { "DISTINCT " } else { "" }, self.get_name(), T::TABLE_NAME),
                    &[],
                ).await?;
            ...
            async fn avg_query(&self, connection: &DatabaseConnection, distinct: bool, condition: QueryCondition<T>) -> crate::Result<$out_type> {
                ...
                let row = connection.query_one(
                    &*format!("SELECT AVG({}{}) FROM {} WHERE {}", if distinct { "DISTINCT " } else { "" }, self.get_name(), T::TABLE_NAME, query),
        ...
        #[async_trait]
        impl<T: Entity<T> + Send + Sync + 'static> AvgColumn<$in_type, $out_type, T> for EntityColumn<'_, Option<$in_type>, T> {
            async fn avg(&self, connection: &DatabaseConnection, distinct: bool) -> crate::Result<$out_type> {
                let row = connection.query_one(
                    &*format!("SELECT AVG({}{}) FROM {}", if distinct { "DISTINCT " } else { "" }, self.get_name(), T::TABLE_NAME),
                    &[],
