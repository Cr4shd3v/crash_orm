...
pub trait MaxColumn<T: ToSql, U: Entity<U>> {
    ...
    async fn max_query(&self, connection: &impl DatabaseConnection, condition: QueryCondition<U>) -> crate::Result<T>;
}
...
macro_rules! impl_max_column {
    ($column_type:ty) => {
        ...
        impl<U: Entity<U> + Sync> MaxColumn<$column_type, U> for EntityColumn<$column_type, U> {
            async fn max(&self, connection: &impl DatabaseConnection) -> crate::Result<$column_type> {
            ...
            async fn max_query(&self, connection: &impl DatabaseConnection, condition: QueryCondition<U>) -> crate::Result<$column_type> {
        ...
        impl<U: Entity<U> + Sync> MaxColumn<Option<$column_type>, U> for EntityColumn<Option<$column_type>, U> {
            async fn max(&self, connection: &impl DatabaseConnection) -> crate::Result<Option<$column_type>> {
                let (query, values, _) = self.get_sql().resolve(1);
            ...
            async fn max_query(&self, connection: &impl DatabaseConnection, condition: QueryCondition<U>) -> crate::Result<Option<$column_type>> {
                let (query, mut values, index) = self.get_sql().resolve(1);
                let (con_query, con_values, _) = condition.resolve(index);
                values.extend(con_values);
