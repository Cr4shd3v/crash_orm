...
async fn test_in() {
    ...
    let results = TestItem14::query(&conn, TestItem14Column::NUMBER.r#in(vec![439, 440])).await;
    println!("{:?}", results);
    assert!(results.is_ok());
    assert_eq!(results.unwrap().len(), 1);

    let results = TestItem14::query(&conn, TestItem14Column::NUMBER.not_in(vec![439, 440])).await;
    assert!(results.is_ok());
    assert_eq!(results.unwrap().len(), 1);

    let results = TestItem14::query(&conn, TestItem14Column::NAME1.r#in(vec![String::from("test12"), String::from("test1234")])).await;
    assert!(results.is_ok());
    assert_eq!(results.unwrap().len(), 1);
